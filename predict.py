...
predicted_cap_state = bottle_cap_state_model_wrapper.predict(args.image, labels = ('opened', 'closed'))

new_classes = []
for i in range(len(classes)):
    new_classes += [f'{predicted_cap_state} {classes[i]}']
...
if args.debug:
    img = bottle.image.plot_segments(img_path=args.image,
                                     ...
                                     mask_alpha=.3,
                                     text_size=1)
    bottle.image.save_image(img, f'{args.image}-debug.jpg')
