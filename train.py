...
parser.add_argument('-lr', '-learning_rate', dest='lr', type = float, default = 1e-3, help='learning rate to use, 0.001 or 1e-3 or .001' )
parser.add_argument('-wd', '-weight_decay', dest='wd', type = float, default = float(0))
...
if args.model == 'bottle-detection':
    ...
    model = bottle.model.build_bottle_cap_state_detection_model(device)
...
train_loss_data = model_wrapper.train(train_dataset=train_loader,
